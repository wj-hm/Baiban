...
getCourseList=()=> {
  ...
  .then(data => {
     let courseList = [{id: 0, name: '不使用课件', subject: 'technology',selectAuth: false}];
...
componentDidMount() {
 ...
 <Button type={type} onClick={this.showModal} size={size}>
        ...
        <Modal
          ...
          footer={[
            // 重点：定义右下角 
            <Button key="submit" type="primary" onClick={this.handleOk} disabled={this.state.chooseList.length==1 ? false : true}>
            确认
            </Button>, ]}
        >
         <div>
             <div>
                 <Button onClick={()=>{this.changeCategory('upload')}}  style={{marginRight:'20px',backgroundColor:folderCategory=='upload' ? 'black' : 'white',color:folderCategory=='upload' ? 'white' : 'black'}}>已上传课件</Button>
                 <Button onClick={()=>{this.changeCategory('bought')}} style={{backgroundColor:folderCategory=='bought' ? 'black' : 'white',color:folderCategory=='bought' ? 'white' : 'black'}} >已购课件</Button>
             </div>
      ...
      enterButton="搜索"
      onSearch={value => { this.selectCourse(value)}}
    />
        </div>
           </div>
      {/* 内容展示 */}
...
<div  key={index} style={{width:'31%',height:100,marginTop:'3.5%',position:'relative'}} onClick={()=>{this.choose(item)}}>
    <div style={{width:'100%',height:100,display:'flex',justifyContent:'flex-start',alignItems:'center',flexWrap:'wrap',backgroundImage:`url(https://static.yi-you.org/project/user-center/image/cover-${this.state.subjectMap[item.subject] ? this.state.subjectMap[item.subject] : 'technology'}.png)`}}>
        <div>{item.name}</div>
    </div>
   {
     item.selectAuth==true?
    <div>
       <div style={{width:'100%',height:'100%',display:'flex',justifyContent:'center',alignItems:'center',backgroundColor:'#666666',opacity:0.7,position:'absolute',top:0,left:0}}>
       <Icon type="check-circle" style={{fontSize:'50px',color:'#27D2D0'}}  />
     ...
     :null
           ...
           <Pagination defaultPageSize={5} onChange={this.selectChange} total={this.state.totalPage} />
